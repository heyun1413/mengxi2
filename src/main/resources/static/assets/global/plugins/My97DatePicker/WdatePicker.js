...
if ($.$crossFrame) {
    try {
        while (U.parent && U.parent[M] != U[M] && U.parent[M][C]("frameset").length == 0) U = U.parent } catch (N) {} }
...
if (!X[M].docMD) { E(X[M], "onmousedown", D);
    X[M].docMD = true }
if (!U[M].docMD) { E(U[M], "onmousedown", D);
    U[M].docMD = true }
E(X, "onunload", function() {
    if ($dp.dd) O($dp.dd, "none") });

function B() { U.$dp = U.$dp || {};
    obj = { $: function($) {
            return (typeof $ == "string") ? X[M].getElementById($) : $ }, $D: function($, _) {
            ...
            O(this.dd, "block") }, hide: function() { O(this.dd, "none") }, attachEvent: E };
        ...
        while (C.tagName && !A.ROOT_TAG.test(C.tagName)) {
            if (C.scrollTop || C.scrollLeft)
                if (!A.OP_SCROLL.test(O(C)))
                    if (!a || C.style.overflow !== "visible") { H -= C.scrollLeft;
                        _ -= C.scrollTop }
            C = C.parentNode }
        if (!E) {
            var $ = Z(I);
...
function T(N, F) { $dp.win = X;
    B();
    N = N || {};
    for (var K in $)
        if (K.substring(0, 1) != "$" && N[K] === undefined) N[K] = $[K];
    if (F) {
        if (!L()) { _ = _ || setInterval(function() {
                if (U[M].readyState == "complete") clearInterval(_);
                T(null, true) }, 50);
            return }
        if ($dp.status == 0) { $dp.status = 1;
            N.el = S;
            I(N, true) } else return } else if (N.eCont) { N.eCont = $dp.$(N.eCont);
        N.el = S;
        N.autoPickDate = true;
        N.qsEnabled = false;
        I(N) } else {
...
function I(G, _) {
    ...
    if (!$dp.dd || G.eCont || ($dp.dd && (G.getRealLang().name != $dp.dd.lang || G.skin != $dp.dd.skin))) {
        ...
        else { $dp.dd = U[M].createElement("DIV");
    ...
    function E(F, E) { F.innerHTML = "<iframe hideFocus=true width=97 height=9 frameborder=0 border=0 scrolling=no></iframe>";
    ...
    function B(I) {
        var G = I.position.left,   //I是控件包装的对象，当其为绝对定位的时候position
            B = I.position.top,
            C = I.el;
        if (C == S) return;
        if (C != I.srcEl && (O(C) == "none" || C.type == "hidden")) C = I.srcEl;
        var H = V(C),  //控件在页面中的绝对位置 top、left、right、bottom、height、width
            $ = F(X),  //iframe滚动扣除外层框架之后的数值，topM为垂直方向滚动距离，为负数，实际就是iframe的也页面相对窗口的offset
            D = L(U),  //视口大小 width、height
            A = Z(U),  //top:500、left:0
            E = $dp.dd.offsetHeight,   //日期框高度
            _ = $dp.dd.offsetWidth;    //日期框宽度
        ...
        // }
        // else {
        //     //B = Math.min(($.topM + H.bottom), (D.height - E)) + 2;
        //     B = H.bottom + 2;
        // }
        if (isNaN(G)) G = 0;
        G += A.left + Math.min($.leftM + H.left, D.width - _ - 5) - (R ? 2 : 0);
        //I.dd.style.position = "fixed";
        I.dd.style.top = B + "px";
